...
MAX_CONCURRENT_TASKS = 16  # 【优化】增加并发数以提高效率
# 【新增】智联招聘登录凭证 (请替换为您的真实信息)
ZHILIAN_USERNAME = "17581758724"  # 你的智联账号
ZHILIAN_PASSWORD = "HXDSG1561958968"  # 你的智联密码

# 城市/省份代码映射表 (用于加速跳转，避免繁琐的UI操作)
# 注意：如果映射表中不存在，将回退到模拟点击操作
CITY_CODE_MAP = {
    "上海": "538",
    "江苏": "635", # 注意：智联搜索通常映射到南京或全省，视具体行为而定，这里使用常见代码
    "浙江": "653", # 杭州/浙江
    "安徽": "664", # 合肥/安徽
    "福建": "673", # 福州/福建
    "江西": "683", # 南昌/江西
...
def scrape_single_combination(keyword: str, province: str, page_size: int) -> List[Dict[str, Any]]:
    ...
    def get_province(page, prov):
        ...
        if prov in ['吉林', '海南']:
            page.ele((By.XPATH, f'//ul[@class="query-other-city__list"]/li[contains(.,"{prov}")]'), timeout=5).click()
        else:
            page.ele((By.XPATH, '//ul[@class="query-other-city__list"]/li[1]'), timeout=5).click()
    ...
    try:
        ...
        # 【优化】尝试直接构造 URL 跳转，如果命中映射表
        city_code = CITY_CODE_MAP.get(province)
        if city_code:
            logger.info(f"[{keyword} | {province}] 命中城市代码 {city_code}，使用极速跳转模式。")
            target_url = f'https://sou.zhaopin.com/?jl={city_code}&kw={keyword}&p=1'
            goto_html(page, target_url)
            # 极速模式下，仍然需要检查登录，但通常已通过全局登录解决
            # handle_login_if_needed(page) # 视情况开启，如果全局登录有效，这里可以注释掉以加速
        else:
            # 未命中映射表，走模拟操作流程
            ...
            logger.info(f"开始处理组合: '{keyword}' - '{province}' (模拟操作模式)")
        ...
        while True:
            ...
            drop_down(page)
            packet = page.listen.wait(timeout=10)  # 【优化】缩短超时时间
...
async def scrape_zhilian_jobs(payload: ScraperPayload):
    ...
    try:
        ...
        # 【新增】全局登录预检
        # 在启动大规模并发前，先用一个任务确保浏览器已登录，避免所有并发任务同时弹出登录框
        logger.info("正在执行全局登录预检...")
        try:
            pre_login_page = ChromiumPage(BROWSER_HOST_PORT).new_tab()
            pre_login_page.get("https://www.zhaopin.com/")
            handle_login_if_needed(pre_login_page)
            time.sleep(1)
            pre_login_page.close()
            logger.info("全局登录预检完成。")
        except Exception as e:
            logger.warning(f"全局登录预检失败 (可能是浏览器未启动或无需登录): {e}")
