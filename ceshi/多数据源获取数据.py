...
# # --- 1. 输入解析模块 ---
# def _parse_input_urls(raw_input: Any) -> List[Dict[str, str]]:
#     """
#     解析来自上游节点(搜索节点)的urls_info输出。
#     输入可以是JSON字符串或已经解析的Python对象。
#     """
#     if isinstance(raw_input, str):
...
#     # 兼容直接传入 urls_info 或包含 urls_info 的字典
#     if isinstance(data, dict) and 'urls_info' in data:
#         urls_info = data['urls_info']
#     elif isinstance(data, list):
#         urls_info = data
#     else:
#         raise ValueError("输入数据中未找到 'urls_info' 列表。")

#     extracted_urls = []
#     if not isinstance(urls_info, list):
#         return []

#     for query_result in urls_info:
#         if not isinstance(query_result, dict): continue
#         web_results = query_result.get("web_results", [])
#         if not isinstance(web_results, list): continue

#         for res in web_results:
#             if not isinstance(res, dict): continue

#             # 动态查找带有 '_url' 和 '_title' 后缀的键
#             url, title, provider = None, None, None
#             for key, value in res.items():
#                 if key.endswith("_url"):
#                     url = value
#                     provider = key.split('_url')[0]
#                     # 尝试找到匹配的title
#                     title_key = f"{provider}_title"
#                     title = res.get(title_key, "Untitled")
#                     break # 找到URL就停止

#             if url and provider:
#                 extracted_urls.append({"url": url, "title": title, "provider": provider})
...
# class JinaScraper(ContentScraper):
#     def __init__(self):
#         self.api_key = os.environ.get("JINA_API_KEY", "jina_b4348ffc39ca47bfbe753b95f59428c7i6ifkOFXRPdF3dRa5Rwb6T8FvrLH")
#         if not self.api_key: raise ValueError("未提供 Jina API Key。")
#         self.base_url = "https://r.jina.ai/"
#         self.headers = {
#             "Authorization": f"Bearer {self.api_key}",
...
# # --- 2.5 ZhiLianJobProvider ---
# class ZhiLianJobProvider(SearchProvider):
#     def __init__(self):
#         self.base_url = "http://119.45.167.133:12906/api/scrape/zhilian"
#         self.headers = {'accept': 'application/json', 'Content-Type': 'application/json'}
#         self.prefix = "zhilian"
#         # 定义一个简单的省市列表用于解析query
#         self.PROVINCES_KEYWORDS = {"北京", "上海", "广州", "深圳", "天津", "重庆", "杭州", "南京", "武汉", "成都", "西安", "苏州", "长沙", "郑州", "东莞", "佛山", "沈阳", "青岛", "济南", "大连", "宁波", "厦门", "福州", "无锡", "合肥", "昆明", "哈尔滨", "长春", "石家庄", "南宁", "贵阳", "乌鲁木齐", "南昌", "兰州", "太原", "海口", "珠海", "惠州", "中山", "汕头", "江门", "湛江", "肇庆", "茂名", "湖北", "广东"}

#     def _parse_query(self, query: str) -> Dict[str, List[str]]:
#         """从查询字符串中解析出省份和关键词。"""
#         parts = query.split()
#         provinces = [p for p in parts if p in self.PROVINCES_KEYWORDS]
#         keywords = [p for p in parts if p not in self.PROVINCES_KEYWORDS]
#         return {"provinces": provinces or [], "keywords": keywords or [query]} # 如果没有关键词，则使用整个query

#     def _format_job_to_markdown(self, job: Dict[str, Any]) -> str:
#         """将单个职位信息格式化为Markdown字符串。"""
#         markdown_parts = [f"## {job.get('name', 'N/A')} - {job.get('companyName', 'N/A')}"]

#         details = {
#             "💰 **薪资**": job.get('salary', '面议'),
#             "🏢 **公司**": f"{job.get('companyName', 'N/A')} ({job.get('propertyName', '未知类型')}, {job.get('companySize', '未知规模')}, {job.get('industryName', '未知行业')})",
#             "📍 **地点**": f"{job.get('province', 'N/A')} {job.get('address', 'N/A')}",
#             "🎓 **要求**": f"{job.get('workingExp', '经验不限')} | {job.get('education', '学历不限')}",
#         }
#         for key, value in details.items():
#             if value: markdown_parts.append(f"- {key}: {value}")

#         if job.get('jobSkillTags'):
#             markdown_parts.append(f"- 🔑 **技能要求**: `{'`, `'.join(job['jobSkillTags'])}`")

#         if job.get('jobKnowledgeWelfareFeatures'):
#             markdown_parts.append(f"- ✨ **福利待遇**: {', '.join(job['jobKnowledgeWelfareFeatures'])}")

#         return "\n".join(markdown_parts)

#     async def search(self, query: str, client: httpx.AsyncClient, num_results: int, search_type: Literal['web', 'video'] = 'web') -> List[Dict[str, Any]]:
#         # 此提供商忽略 search_type
#         print(f"🔗 [ZhiLian Scraper] 开始处理查询: {query}")
#         try:
#             parsed_q = self._parse_query(query)
#             payload = {
...
#             results = []
#             for job in job_list:
#                 content_md = self._format_job_to_markdown(job)

#                 # 创建唯一的伪URL
#                 company_sanitized = re.sub(r'[^a-zA-Z0-9]', '-', job.get('companyName', ''))
#                 name_sanitized = re.sub(r'[^a-zA-Z0-9]', '-', job.get('name', ''))
#                 pseudo_url = f"zhilian-job://{company_sanitized}/{name_sanitized}"

#                 formatted_item = {
#                     "type": "job",
#                     "url": pseudo_url,
#                     "title": f"{job.get('name', 'N/A')} - {job.get('companyName', 'N/A')}",
#                     "snippet": content_md,
#                     "content": content_md, # 因为直接获取了内容，snippet和content可以一样
#                     "raw_data": job # 保留原始数据以备后用
#                 }
#                 results.append(self._prefix_keys(formatted_item, self.prefix))

#             print(f"✅ [ZhiLian Scraper] 成功: 找到 {len(results)} 条职位信息 for '{query}'")
#             return results

#         except Exception as e:
#             error_msg = f"处理失败 for '{query}': {type(e).__name__} - {e}"
#             print(f"⚠️ [ZhiLian Scraper] {error_msg}")
#             # 返回符合框架期望的错误格式
#             return [self._prefix_keys({"type": "job", "error": f"ZhiLian Job API request failed for '{query}': {e}"}, self.prefix)]


# # --- 3. 统一调度中心 ---
# class ContentOrchestrator:
#     def __init__(self):
#         self.scrapers: Dict[str, ContentScraper] = {
...
#     async def process_urls(self, url_list: List[Dict[str, str]]) -> List[Dict[str, Any]]:
#         # HTTPS 设置，忽略 SSL 验证错误，增加稳定性
#         ssl_context = httpx.create_ssl_context(verify=False)
#         async with httpx.AsyncClient(http2=True, verify=ssl_context, timeout=30, follow_redirects=True, limits=httpx.Limits(max_connections=50, max_keepalive_connections=10)) as client:
#             tasks = []
#             for item in url_list:
#                 provider = item.get("provider")
#                 scraper = self.scrapers.get(provider)
#                 if scraper:
#                     tasks.append(scraper.scrape(item["url"], item["title"], client))
#                 else:
#                     print(f"❓ 未找到提供商 '{provider}' 的抓取器, 跳过 {item['url']}")

#             results = await asyncio.gather(*tasks, return_exceptions=True)
#             return results

# # --- 4. Dify 节点主入口 ---
# async def main_async(raw_urls_info: Any) -> Dict[str, Any]:
#     # 1. 解析输入
#     url_list = _parse_input_urls(raw_urls_info)
#     if not url_list:
#         print("🟡 输入中没有有效的URL，提前返回。")
#         return {"scraped_data": [], "scraped_data_str": "[]"}

#     # 2. 实例化并运行调度器
#     orchestrator = ContentOrchestrator()
#     scraped_results = await orchestrator.process_urls(url_list)

#     # 3. 格式化输出
#     final_structured_data = []
#     for result in scraped_results:
#         if isinstance(result, Exception):
#             print(f"‼️ 任务处理时发生严重异常: {result}")
#             continue
...
#             # 创建一个唯一的 source_id
#             sanitized_url = re.sub(r'[^a-zA-Z0-9]', '-', url.replace("https://", "").replace("http://", ""))
...
#     return {
#         "scraped_data": final_structured_data,
#         "scraped_data_str": json.dumps(final_structured_data, ensure_ascii=False, indent=2)
#     }

# def main(urls_info: Any) -> Dict[str, Any]:
#     """
#     Dify 同步入口: 接收上游搜索节点的输出，为每个URL提取详细内容。
#     - urls_info: 来自搜索节点的输出变量，可以是JSON字符串或对象。
#     """
#     try:
#         return asyncio.run(main_async(raw_urls_info=urls_info))
#     except Exception as e:
#         print(f"‼️ 节点执行时发生顶层错误: {e}")
#         error_payload = [
#             {
#                 "source_id": "NODE_EXECUTION_ERROR",
#                 "source_name": "节点执行失败",
#                 "url": "",
#                 "content": f"An error occurred in the node: {str(e)}\n\n{traceback.format_exc()}"
#             }
#         ]
#         return {
#             "scraped_data": error_payload,
#             "scraped_data_str": json.dumps(error_payload, ensure_ascii=False, indent=2)
#         }
