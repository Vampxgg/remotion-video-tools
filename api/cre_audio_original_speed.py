# # -*- coding: utf-8 -*-
# # @File：cre_audio_original_speed.py
# # @Time：2025/9/7 16:38
# # @Author：_不咬闰土的猹丶
...
#
#         if not results:
#             logger.warning("在整个文档中未能找到任何有效的 '<subtitle>' 与其相邻的时间戳组合。")
#         else:
...
#                 task_info['actual_duration_sec'] = 0.0  # 记录实际时长
#             else:
#                 target_duration_ms = int(target_duration_sec * 1000)
#                 logger.info(f"[Task {task_id}] 检测到静音任务，在本地生成 {target_duration_ms}ms 的静音。")
#                 silence = AudioSegment.silent(duration=target_duration_ms)
#                 silence.export(full_audio_path, format=AUDIO_FORMAT)
#                 task_info['actual_duration_sec'] = target_duration_sec  # 记录实际时长
#
...
#             try:
#                 # --- [核心简化逻辑] ---
#                 # 直接将完整文本提交给 TTS 服务，并将音频流直接写入最终文件。
#                 logger.debug(f"[Task {task_id}] 开始直接生成完整音频，无分片处理。")
#
#                 req = TTSRequest(
#                     text=subtitle_text,
...
#                     model=ENGINE_MODEL,
#                     format=AUDIO_FORMAT,
#                     prosody=Prosody(speed=0.9)  # 语速可按需保留或设为1.0
#                 )
#
...
#                     raise ValueError("TTS API返回了空音频流。")
#
#                 audio_buffer.seek(0)  # 重置缓冲区指针以供读取
#
#                 # 步骤 2: 使用 pydub 从内存加载音频并添加静音
#                 tts_audio = AudioSegment.from_file(audio_buffer, format=AUDIO_FORMAT)
#                 one_sec_silence = AudioSegment.silent(duration=1000)  # 1000ms = 1s
#                 padded_audio = one_sec_silence + tts_audio + one_sec_silence
#                 logger.info(f"[Task {task_id}] 已为音频前后添加1秒静音。")
#
#                 # 步骤 3: 计算添加静音后的总时长
#                 actual_duration_sec = len(padded_audio) / 1000.0
#                 task_info['actual_duration_sec'] = actual_duration_sec
#                 logger.info(f"[Task {task_id}] 计算出新音频时长为: {actual_duration_sec:.2f} 秒。")
#
#                 # 步骤 4: 将处理后的音频保存到文件
#                 padded_audio.export(full_audio_path, format=AUDIO_FORMAT)
#
#                 # # 直接将音频流写入最终文件路径
...
#                 #         f.write(chunk)
#                 #
#                 # # 检查生成的文件是否有效（防止API返回空内容）
#                 # if os.path.getsize(full_audio_path) == 0:
#                 #     raise ValueError("生成的音频文件为空。")
#
#                 # 无需任何后处理，直接标记成功
#                 task_info["audio_path"] = task_info["public_url"]
#                 logger.info(f"[Task {task_id}] TTS音频生成成功 (原始版本，无加速或填充)。")
#                 break  # 成功完成，跳出重试循环
#
#             except Exception as e:
...
#                 if len(time_parts) != 2:
#                     raise ValueError("时间范围格式不正确，缺少 '-' 分隔符。")
#
...
#         # return final_result
#
#         # 步骤 1: 必须按原始顺序排序，以确保时间轴是连续的
#         final_tasks_list.sort(key=lambda x: x['id'])
#
#         # 步骤 2: 重新计算时间轴
#         logger.info("所有音频任务处理完成，开始根据实际音频时长重新计算时间轴...")
#         current_time_cursor_sec = 0.0
#         for task_result in final_tasks_list:
#             # 获取任务返回的实际时长，如果任务失败则为0
#             duration = task_result.get('actual_duration_sec', 0.0)
#
#             # 计算新的开始和结束时间
#             start_sec = current_time_cursor_sec
#             end_sec = start_sec + duration
#
#             # 添加新的字段到结果中
#             task_result["modify_start_sec"] = round(start_sec, 3)
#             task_result["modify_end_sec"] = round(end_sec, 3)
#             task_result["modify_time_range"] = f"{start_sec:.3f}秒 - {end_sec:.3f}秒"
#
#             # 更新时间光标，为下一个片段做准备
#             current_time_cursor_sec = end_sec
#
#             # (可选) 清理不再需要的中间字段
#             task_result.pop('actual_duration_sec', None)
#
#         logger.info("新的同步时间轴计算完成。")
#
#         # 步骤 3: 移动文件
#         move_error = None
#         if any(task.get("audio_path") for task in final_tasks_list):
...
#             "audio_tasks": final_tasks_list,
#             "audio_tasks_str": json.dumps(final_tasks_list, ensure_ascii=False)
#             # indent=2 for better readability
#         }
#         if move_error:
#             final_result["move_operation_error"] = move_error
#
