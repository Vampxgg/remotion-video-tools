...
| `GET` | `/api/tianyancha/companies` | 本地企业库查询；显式补详情时可调用远程 | 默认否 |
...
| `TIANYANCHA_MAX_DETAIL_CALLS_PER_REQUEST` | `20` | 普通搜索单次最多补拉详情数量 |
| `TIANYANCHA_DIFY_DEFAULT_LIMIT` | `20` | Dify 区域调研默认返回数 |
| `TIANYANCHA_DIFY_MAX_LIMIT` | `50` | Dify 区域调研最大返回数 |
| `TIANYANCHA_DIFY_MAX_DETAIL_CALLS_PER_REQUEST` | `50` | Dify 详情模式单次最多补拉详情数量 |
| `TIANYANCHA_ENRICH_NEW_COMPANIES` | `true` | 远程搜索发现新公司时是否自动补拉详情 |
...
4. 将搜索结果去重写入本地企业库。
5. 如果 `enrich_detail=true`，按限制补拉企业基本信息；如果 `TIANYANCHA_ENRICH_NEW_COMPANIES=true`，远程搜索中新入库的企业也会优先自动补详情。
...
| `enrich_detail` | bool | 否 | `false` | 是否对本页企业补拉缺失或过期的基本信息 |
| `force_remote` | bool | 否 | `false` | 是否跳过本地搜索缓存 |
| `refresh_detail` | bool | 否 | `false` | 补详情时是否忽略详情 TTL |
| `max_detail_calls` | int / null | 否 | `null` | 本次最多补拉详情数量，普通搜索最大默认 `20`；传 `0` 可禁止本次补详情 |
...
查询本地已入库企业。默认不调用天眼查远程接口，适合低成本检索、后台列表、二次筛选；只有显式传 `enrich_detail=true` 时，才会对返回企业中缺失或过期的基本信息进行补拉。
...
| `enrich_detail` | bool | `false` | 是否为返回企业补拉缺失或过期的基本信息 |
| `refresh_detail` | bool | `false` | 补详情时是否忽略详情 TTL |
| `max_detail_calls` | int / null | `null` | 本次最多补拉详情数量，最大默认 `20` |
...
如果希望对返回企业补拉详情，必须显式开启：

```bash
curl "http://127.0.0.1:2906/api/tianyancha/companies?keyword=百度&limit=20&enrich_detail=true&max_detail_calls=3" \
  -H "X-API-Key: your-api-key"
```
...
| `company_type` | 天眼查机构类型编码 |
| `company_org_type` | 企业组织类型 |
| `legal_type` | 天眼查法人类型编码 |
| `base` | 省份或地区 |
| `city` | 城市 |
| `district` | 区县 |
| `district_code` | 区县代码 |
| `industry` | 行业 |
| `category` | 行业大类 |
| `category_code_first` | 行业门类代码 |
| `category_code_second` | 行业大类代码 |
| `category_code_third` | 行业中类代码 |
| `category_code_fourth` | 行业小类代码 |
| `established_at` | 成立日期 |
| `approved_at` | 核准日期 |
| `from_time` | 经营开始日期 |
| `to_time` | 经营结束日期 |
| `updated_remote_at` | 天眼查侧更新时间 |
| `reg_institute` | 登记机关 |
| `business_scope` | 经营范围 |
| `reg_location` | 注册地址 |
| `staff_num_range` | 人员规模 |
| `social_staff_num` | 参保人数 |
| `tags` | 标签 |
| `history_names` | 曾用名 |
| `percentile_score` | 企业评分 |
| `is_micro_ent` | 是否小微企业 |
...
- 默认 TTL：`TIANYANCHA_BASEINFO_TTL_DAYS=3650`，约 10 年
- 未过期时优先使用本地数据。
- `force_remote=true` 可强制刷新单个企业。
- 搜索接口中 `refresh_detail=true` 可强制刷新补拉详情。
- 远程搜索新入库企业会在 `TIANYANCHA_ENRICH_NEW_COMPANIES=true` 时优先自动补详情；已有企业只在详情缺失或过期时补拉。
- `max_detail_calls=0` 会禁止本次补详情，适合只想拉搜索摘要且不希望产生详情成本的场景。
