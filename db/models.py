...
class TianyanchaSearchQuery(Base):
    ...
    page_num = Column(Integer, nullable=False, default=1, comment="页码（最近一次远程翻页）")
    page_size = Column(Integer, nullable=False, default=20, comment="每页条数")
    max_page_fetched = Column(Integer, nullable=False, default=0, comment="已翻到的最大页码（企业池语义的进度）")
    exhausted = Column(Boolean, nullable=False, default=False, comment="企业池是否已翻完")
