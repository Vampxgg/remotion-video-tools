...
async def _process_images(
    ...
    # 先拿字节（并发、受 deadline 约束）。
    async def _fetch(img: ImageElement) -> Tuple[ImageElement, Optional[Tuple[bytes, str]]]:
        if time.time() >= deadline_at - 1.0:
            return img, None
        async with sem:
            return img, await _ensure_bytes(img)

    fetched = await asyncio.gather(*[_fetch(im) for im in images], return_exceptions=True)
    ...
    await asyncio.gather(*[_recog(im) for im in rep_images], return_exceptions=True)
...
async def _process_tables(
    ...
    await asyncio.gather(*[_proof(t) for t in targets], return_exceptions=True)
